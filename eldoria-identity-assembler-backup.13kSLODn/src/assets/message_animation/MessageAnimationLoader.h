...
public:
  ...
  const MessageAnimationResource &resource(std::uint16_t id) const;
...
private:
  ...
  MessageAnimationAssembler assembler_;

  mutable std::unordered_map<std::uint16_t, MessageAnimationData> dataCache_;

  mutable std::unordered_map<std::uint16_t, MessageAnimationResource>
      resourceCache_;
};
