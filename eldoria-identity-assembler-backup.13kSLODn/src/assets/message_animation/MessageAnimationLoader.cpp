...
MessageAnimationLoader::MessageAnimationLoader(const eld::cache::Cache &cache)
    : archive_(eld::archive::load(cache.open(Index), ArchiveId)) {}

MessageAnimationData MessageAnimationLoader::loadData(std::uint16_t id) const {
  ...
  if (dataCount != indexCount) {
    throw std::runtime_error("Message-animation data/index count mismatch");
  }

  if (id >= dataCount) {
    throw std::out_of_range("Message-animation does not exist");
  }
  ...
  for (std::uint16_t currentId = 0; currentId < id; ++currentId) {
    ...
    if (!dataReader.canRead(size)) {
      throw std::runtime_error("Message-animation asset exceeds data file");
    }
  ...
  try {
    MessageAnimationData asset = decoder_.decode(data);

    asset.id = id;

    return asset;
  } catch (const std::exception &error) {
    throw std::runtime_error("Failed to decode message-animation " +
                             std::to_string(id) + ": " + error.what());
  }
}

std::optional<MessageAnimationData>
MessageAnimationLoader::find(std::uint16_t id) const {
  if (!contains(id)) {
    return std::nullopt;
  }
...
std::vector<std::uint16_t> MessageAnimationLoader::listIds() const {
...
bool MessageAnimationLoader::contains(std::uint16_t id) const {
  return id < count();
}

std::size_t MessageAnimationLoader::count() const {
  const eld::archive::ArchiveFile &dataFile = archive_.get(DataFile);
...
const MessageAnimationData &
MessageAnimationLoader::data(std::uint16_t id) const {
...
const MessageAnimationResource &
MessageAnimationLoader::resource(std::uint16_t id) const {
