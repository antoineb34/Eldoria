...
public:
  ...
  const IdentityKitResource &resource(std::uint16_t id) const;
...
private:
  ...
  IdentityKitAssembler assembler_;

  mutable std::unordered_map<std::uint16_t, IdentityKitData> dataCache_;

  mutable std::unordered_map<std::uint16_t, IdentityKitResource> resourceCache_;
};
