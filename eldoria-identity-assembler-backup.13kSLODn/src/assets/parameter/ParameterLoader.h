...
public:
  ...
  const ParameterResource &resource(std::uint16_t id) const;
...
private:
  ...
  ParameterAssembler assembler_;

  mutable std::unordered_map<std::uint16_t, ParameterData> dataCache_;

  mutable std::unordered_map<std::uint16_t, ParameterResource> resourceCache_;
};
