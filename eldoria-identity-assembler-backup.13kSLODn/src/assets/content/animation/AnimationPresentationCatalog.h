...
public:
  ...
  explicit AnimationPresentationCatalog(const std::filesystem::path &path);

  void load(const std::filesystem::path &path);

  NpcAnimationProfile resolveNpc(const eld::npc::NpcData &definition) const;

  ItemAnimationProfile resolveItem(const eld::item::ItemData &definition) const;

private:
  enum class EntityKind : std::uint8_t { Npc, Item };
