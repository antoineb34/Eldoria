...
std::optional<std::uint16_t> parseU16(const std::string &text) {
...
bool parseBool(std::string_view text) {
  return text == "1" || text == "true" || text == "yes" || text == "projectile";
}

void addDefinitionBinding(std::vector<AnimationBinding> &bindings,
...
void mergeBinding(std::vector<AnimationBinding> &bindings,
  ...
  existing->effects.insert(existing->effects.end(), authored.effects.begin(),
...
void AnimationPresentationCatalog::load(const std::filesystem::path &path) {
  authored_.clear();
  ...
  while (std::getline(input, line)) {
    ...
    if (line.empty() || line.front() == '#' || line.rfind("kind,", 0) == 0) {
      continue;
    }

    const std::vector<std::string> fields = splitCsv(line);

    const std::optional<std::uint16_t> entityId = parseU16(fields[1]);

    const std::optional<AnimationAction> action =
        animationActionFromString(fields[2]);

    if (!entityId.has_value() || !action.has_value()) {
      continue;
    }
    ...
    const Key key{kind, *entityId, *action, fields[8]};

    AnimationBinding &binding = authored_[key];

    binding.action = *action;
    binding.variant = fields[8];

    const std::optional<std::uint16_t> sequenceId = parseU16(fields[3]);

    if (sequenceId.has_value()) {
      binding.sequenceId = sequenceId;
    }

    const std::optional<std::uint16_t> spotAnimationId = parseU16(fields[4]);
    ...
    if (spotAnimationId.has_value()) {
      ...
      } else if (fields[9] == "source" || fields[9] == "attached") {
        effect.projectile = false;
      }
      ...
      if (const auto duration = parseU32(fields[7])) {
        effect.durationMilliseconds = std::max<std::uint32_t>(*duration, 1u);
      }
...
NpcAnimationProfile AnimationPresentationCatalog::resolveNpc(
    const eld::npc::NpcData &definition) const {
  NpcAnimationProfile profile;
  profile.npcId = definition.id;

  addDefinitionBinding(profile.bindings, AnimationAction::Idle,
                       definition.idleAnimationId);

  addDefinitionBinding(profile.bindings, AnimationAction::Walk,
                       definition.walkAnimationId);

  addDefinitionBinding(profile.bindings, AnimationAction::TurnAround,
                       definition.turnAroundAnimationId);

  addDefinitionBinding(profile.bindings, AnimationAction::TurnLeft,
                       definition.turnLeftAnimationId);

  addDefinitionBinding(profile.bindings, AnimationAction::TurnRight,
                       definition.turnRightAnimationId);

  for (const auto &[key, binding] : authored_) {
    if (key.kind == EntityKind::Npc && key.id == definition.id) {
      mergeBinding(profile.bindings, binding);
    }
  }
...
ItemAnimationProfile AnimationPresentationCatalog::resolveItem(
    const eld::item::ItemData &definition) const {
  ItemAnimationProfile profile;
  profile.itemId = definition.id;

  for (const auto &[key, binding] : authored_) {
    if (key.kind == EntityKind::Item && key.id == definition.id) {
      mergeBinding(profile.bindings, binding);
    }
  }
