...
VarpLoader::VarpLoader(const eld::cache::Cache &cache)
    : archive_(eld::archive::load(cache.open(Index), ArchiveId)) {}

VarpData VarpLoader::loadData(std::uint16_t id) const {
  ...
  if (dataCount != indexCount) {
    throw std::runtime_error("varp data/index count mismatch");
  }

  if (id >= dataCount) {
    throw std::out_of_range("varp does not exist");
  }
  ...
  for (std::uint16_t currentId = 0; currentId < id; ++currentId) {
    ...
    if (!dataReader.canRead(size)) {
      throw std::runtime_error("varp asset exceeds data file");
    }
  ...
  try {
    VarpData asset = decoder_.decode(data);

    asset.id = id;

    return asset;
  } catch (const std::exception &error) {
    throw std::runtime_error("Failed to decode varp " + std::to_string(id) +
                             ": " + error.what());
  }
}

std::optional<VarpData> VarpLoader::find(std::uint16_t id) const {
  if (!contains(id)) {
    return std::nullopt;
  }
...
std::vector<std::uint16_t> VarpLoader::listIds() const {
...
bool VarpLoader::contains(std::uint16_t id) const { return id < count(); }

std::size_t VarpLoader::count() const {
  const eld::archive::ArchiveFile &dataFile = archive_.get(DataFile);
...
const VarpData &VarpLoader::data(std::uint16_t id) const {
...
const VarpResource &VarpLoader::resource(std::uint16_t id) const {
