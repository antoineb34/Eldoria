...
SequenceLoader::SequenceLoader(
    ...
    : SequenceLoader(cache) {
    assembler_ = SequenceAssembler(frames);
}


const SequenceData& SequenceLoader::data(
    std::uint16_t id
) const {
    const auto existing =
        dataCache_.find(id);
...
SequenceData SequenceLoader::loadData(
    std::uint16_t id
) const {
...
std::size_t SequenceLoader::count() const {
    const eld::archive::ArchiveFile& dataFile =
        archive_.get(DataFile);
