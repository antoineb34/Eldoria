...
void prepareMesh(eld::model::ModelData &mesh,
  ...
  for (eld::model::Vertex &vertex : mesh.vertices) {
    ...
    vertex.z = vertex.z * static_cast<float>(definition.scaleX) / 128.0f;
  }

  for (eld::model::Face &face : mesh.faces) {
    for (const eld::npc::NpcRecolor &recolor : definition.recolors) {
...
NpcView::build(const eld::npc::NpcData &definition,
