...
IdentityKitView::build(const eld::identity_kit::IdentityKitData &definition,
                       const eld::model::ModelLoader &repository) const {
  std::vector<std::uint16_t> modelIds = definition.modelIds;

  if (modelIds.empty()) {
    for (const std::optional<std::uint16_t> &modelId :
  ...
  for (const std::uint16_t modelId : modelIds) {
    ...
    applyRecolors(*model, definition);
