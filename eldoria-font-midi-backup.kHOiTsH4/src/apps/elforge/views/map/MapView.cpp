...
TerrainNeighborhood
loadTerrainNeighborhood(const eld::map::MapRepository &loader,
  ...
  for (int offsetX = -1; offsetX <= 1; ++offsetX) {
    for (int offsetY = -1; offsetY <= 1; ++offsetY) {
      ...
      result.regions.push_back(
          {offsetX, offsetY, loader.loadTerrain(candidateId)});
    }
  }
...
float averagePlaneHeight(const eld::map::MapRegion &region, std::size_t plane) {
  std::int64_t sum = 0;
  std::size_t count = 0;
...
MapView::MapView(const eld::map::MapRepository &loader,
...
MapViewState MapView::build(std::uint16_t regionId) const {
  ...
  viewState.centerRegion = loader_.get(regionId);
