...
void appendMesh(eld::model::ModelData &destination,
  ...
  destination.faces.insert(destination.faces.end(), source.faces.begin(),
                           source.faces.end());

  destination.textureMappings.insert(destination.textureMappings.end(),
                                     source.textureMappings.begin(),
                                     source.textureMappings.end());
}

void applyItemRecolors(eld::model::ModelData &mesh,
...
void applyIdentityKitRecolors(
  ...
  for (eld::model::Face &face : mesh.faces) {
    for (std::size_t index = 0; index < definition.recolorSources.size();
         ++index) {
      if (definition.recolorSources[index].has_value() &&
...
void hideWearPosition(HiddenBobParts &hidden,
...
HiddenBobParts hiddenBodyParts(const eld::item::ItemData &definition) {
  HiddenBobParts hidden{};

  bool hasExactWearData = false;

  for (const std::optional<eld::item::ItemWearPosition> &position :
       {definition.wearPosition, definition.wearPosition2,
  ...
  if (containsAny(name, {"platebody", "chainbody", "body", "robe top", "shirt",
                         "jacket"})) {
    hidden[static_cast<std::size_t>(BobPart::Torso)] = true;
    hidden[static_cast<std::size_t>(BobPart::Arms)] = true;
  }

  if (containsAny(name, {"platelegs", "legs", "trousers", "skirt", "chaps"})) {
    hidden[static_cast<std::size_t>(BobPart::Legs)] = true;
  }
...
defaultIdentityKit(std::uint8_t bodyPartId,
                   const eld::identity_kit::IdentityKitLoader &repository) {
  for (std::uint16_t id : repository.listIds()) {
    const eld::identity_kit::IdentityKitData &kit = repository.data(id);
    if (kit.selectable && kit.bodyPartId.has_value() &&
        *kit.bodyPartId == bodyPartId && !kit.modelIds.empty()) {
      return &kit;
    }
  }
...
ItemView::build(const eld::item::ItemData &definition,
  ...
  for (eld::model::Vertex &vertex : model->vertices) {
    vertex.x = vertex.x * static_cast<float>(definition.scaleX) / 128.0f;
    vertex.y = vertex.y * static_cast<float>(definition.scaleY) / 128.0f;
    vertex.z = vertex.z * static_cast<float>(definition.scaleZ) / 128.0f;
  }
...
bool ItemView::hasEquippedModel(const eld::item::ItemData &definition,
                                ItemViewGender gender) const {
  const auto &ids = gender == ItemViewGender::Male ? definition.maleModelIds
                                                   : definition.femaleModelIds;

  return std::any_of(
      ids.begin(), ids.end(),
      [](const std::optional<std::uint16_t> &id) { return id.has_value(); });
}

std::optional<eld::model::ModelData> ItemView::buildEquipped(
    const eld::item::ItemData &definition, ItemViewGender gender,
  ...
  const std::uint8_t genderOffset = gender == ItemViewGender::Male ? 0 : 7;
  ...
  for (std::size_t part = 0; part < hidden.size(); ++part) {
    ...
    const auto *kit = defaultIdentityKit(
        static_cast<std::uint8_t>(part) + genderOffset, identityKits);
  ...
  const auto &equippedIds = gender == ItemViewGender::Male
                                ? definition.maleModelIds
                                : definition.femaleModelIds;

  const std::int8_t verticalOffset = gender == ItemViewGender::Male
