...
bool itemHasInventoryAction(const eld::item::ItemData &definition,
                            const std::string &wanted) {
  const std::string wantedLower = lowerAscii(wanted);

  return std::any_of(definition.inventoryActions.begin(),
...
bool itemHasWearPosition(const eld::item::ItemData &definition,
                         eld::item::ItemWearPosition wanted) {
  return definition.wearPosition == wanted ||
         definition.wearPosition2 == wanted ||
         definition.wearPosition3 == wanted;
}

bool itemLooksLikeShield(const eld::item::ItemData &definition) {
  if (itemHasWearPosition(definition, eld::item::ItemWearPosition::LeftHand)) {
    return true;
  }
...
bool itemLooksLikeWeapon(const eld::item::ItemData &definition) {
  if (itemLooksLikeShield(definition)) {
    return false;
  }

  if (itemHasWearPosition(definition, eld::item::ItemWearPosition::RightHand)) {
    return true;
  }
...
makeItemActionBinding(eld::animation::presentation::AnimationAction action,
                      const char *variant, std::uint16_t sequenceId) {
...
genericItemActionBindings(const eld::item::ItemData &definition) {
  using Action = eld::animation::presentation::AnimationAction;
  using Binding = eld::animation::presentation::AnimationBinding;

  std::vector<Binding> bindings;
  const std::string name = lowerAscii(definition.name);

  const auto add = [&](Action action, const char *variant,
                       std::uint16_t sequenceId) {
    bindings.push_back(makeItemActionBinding(action, variant, sequenceId));
  };
  ...
  if (itemLooksLikeWeapon(definition)) {
    ...
    } else if (name.find("javelin") != std::string::npos ||
    ...
    } else if (name.find("2h") != std::string::npos ||
    ...
    } else if (name.find("mace") != std::string::npos ||
...
genericItemDefendSequence(const eld::item::ItemData &definition) {
  const std::string name = lowerAscii(definition.name);
  ...
  if (name.find("2h") != std::string::npos ||
      name.find("two-handed") != std::string::npos ||
      name.find("two handed") != std::string::npos) {
    return 410; // human_dhsword_block
  }

  if (name.find("mace") != std::string::npos ||
      name.find("maul") != std::string::npos ||
      name.find("hammer") != std::string::npos) {
...
std::vector<eld::animation::presentation::AnimationBinding>
ItemAnimationPresets::actions(const eld::item::ItemData &definition) {
  return genericItemActionBindings(definition);
}

std::optional<std::uint16_t>
ItemAnimationPresets::defendSequence(const eld::item::ItemData &definition) {
