...
void CacheExplorer::handleSelectionChanged() {
  ...
  case CacheTreeNodeType::Animation: {
    if (state_.selection.fileId < 0 ||
        state_.selection.fileId > std::numeric_limits<std::uint16_t>::max()) {
      break;
    }
    ...
    try {
      ...
      state_.activeAnimation = relations.inspect(
      ...
      state_.animationExportStatus =
          std::string("Failed to inspect animation: ") + exception.what();
    }
  ...
  case CacheTreeNodeType::Midi: {
    ...
    try {
      state_.activeMidi = midiRepository_.get(
          static_cast<std::uint16_t>(state_.selection.fileId));

      if (midiPlayer_.isAvailable()) {
        if (midiPlayer_.load(state_.activeMidi->bytes)) {
          state_.midiView.playbackStatus = "Ready to play";
        } else {
      ...
      const std::string message =
          std::string("Failed to load MIDI: ") + exception.what();
  ...
  case CacheTreeNodeType::MapRegion: {
    if (state_.selection.regionId < 0 ||
        state_.selection.regionId > std::numeric_limits<std::uint16_t>::max()) {
      state_.mapViewError = "Selected map region id is invalid";
      break;
    }

    try {
      const MapView view(mapRepository_, floorLoader_, locationLoader_,
                         modelLoader_, graphicsResources_);

      state_.activeMap =
          view.build(static_cast<std::uint16_t>(state_.selection.regionId));

      resetMapView(state_.mapPlane, state_.mapYaw, state_.mapPitch,
                   state_.mapDistance);
      ...
      state_.activeMap.reset();
      state_.mapViewError = exception.what();
    }
  ...
  case CacheTreeNodeType::Font: {
    if (state_.selection.archiveId != 1 || state_.selection.fileId < 0 ||
        state_.selection.fileId > std::numeric_limits<std::uint16_t>::max()) {
      break;
    }

    state_.activeFont = titleFontRepository_.find(
        static_cast<std::uint16_t>(state_.selection.fileId));

    if (state_.activeFont.has_value()) {
      const FontView view;

      state_.activeImage = view.build(*state_.activeFont);
    }
  ...
  case CacheTreeNodeType::Widget: {
    if (state_.selection.definitionId >= 0) {
      const auto definition = widgetLoader_.find(
          static_cast<std::uint16_t>(state_.selection.definitionId));

      if (definition) {
        state_.activeInterface = *definition;

        state_.activeInterfaceDump =
            InterfaceInspector::inspect(*definition, widgetLoader_);
  ...
  case CacheTreeNodeType::Message: {
    if (state_.selection.definitionId >= 0) {
      const auto definition = messageLoader_.find(
          static_cast<std::uint16_t>(state_.selection.definitionId));
  ...
  case CacheTreeNodeType::MessageAnimation: {
    if (state_.selection.definitionId >= 0) {
      const auto definition = messageAnimationLoader_.find(
          static_cast<std::uint16_t>(state_.selection.definitionId));

      if (definition.has_value()) {
        state_.activeMessageAnimation = *definition;
      }
    }
  ...
  case CacheTreeNodeType::Parameter: {
    if (state_.selection.definitionId >= 0) {
      const auto definition = parameterLoader_.find(
          static_cast<std::uint16_t>(state_.selection.definitionId));

      if (definition) {
        state_.activeParameter = *definition;
      }
    }
  ...
  case CacheTreeNodeType::Varp: {
    if (state_.selection.definitionId >= 0) {
      const auto definition = varpLoader_.find(
          static_cast<std::uint16_t>(state_.selection.definitionId));
  ...
  case CacheTreeNodeType::Varbit: {
    if (state_.selection.definitionId >= 0) {
      const auto definition = varbitLoader_.find(
          static_cast<std::uint16_t>(state_.selection.definitionId));
  ...
  case CacheTreeNodeType::SpotAnimation: {
    ...
    const auto definition = spotAnimationLoader_.find(
        static_cast<std::uint16_t>(state_.selection.definitionId));
    ...
    if (definition) {
      ...
      if (model.has_value()) {
        ...
        if (animationSource.has_value()) {
          ...
          startAnimationView(definition->sequenceId);
        }
      }
    }
  ...
  case CacheTreeNodeType::Sequence: {
    ...
    const auto definition = sequenceLoader_.find(
        static_cast<std::uint16_t>(state_.selection.definitionId));

    if (definition) {
      state_.activeSequence = *definition;
    }
  ...
  case CacheTreeNodeType::Item: {
    ...
    const auto definition = itemLoader_.find(
        static_cast<std::uint16_t>(state_.selection.definitionId));
  ...
  case CacheTreeNodeType::Npc: {
    ...
    const auto definition = npcLoader_.find(
        static_cast<std::uint16_t>(state_.selection.definitionId));
  ...
  case CacheTreeNodeType::Location: {
    ...
    const auto definition = locationLoader_.find(
        static_cast<std::uint16_t>(state_.selection.definitionId));
    ...
    if (definition.has_value()) {
      ...
      if (model.has_value()) {
        ...
        if (animationSource.has_value()) {
          ...
          startAnimationView(definition->animationId);
        }
      }
    }
  ...
  case CacheTreeNodeType::IdentityKit: {
    ...
    const auto definition = identityKitLoader_.find(
        static_cast<std::uint16_t>(state_.selection.definitionId));

    if (definition.has_value()) {
      state_.activeIdentityKit = *definition;
  ...
  case CacheTreeNodeType::Floor: {
    ...
    const auto floor = floorLoader_.find(
        static_cast<std::uint16_t>(state_.selection.definitionId));
    ...
    if (floor.has_value()) {
      ...
      if (!state_.activeTexture.has_value() &&
          (floor->rgb.has_value() || floor->secondaryRgb.has_value())) {
        state_.activeImage = FloorView{}.build(*floor);
      }
    }
  ...
  case CacheTreeNodeType::Image: {
    if (state_.selection.archiveId == 1 && !state_.selection.name.empty()) {
      state_.activeImage = titleLoader_.find(state_.selection.name);
    }
  ...
  case CacheTreeNodeType::SpriteFrame: {
    const int selectedFrame =
        state_.selection.frameId >= 0 ? state_.selection.frameId : 0;

    if (selectedFrame > std::numeric_limits<std::uint16_t>::max()) {
      break;
    }

    const auto frameId = static_cast<std::uint16_t>(selectedFrame);

    if (state_.selection.archiveId == 1 && !state_.selection.name.empty()) {
      state_.activeSprite =
          titleSpriteLoader_.find(state_.selection.name, frameId);
    } else if (state_.selection.archiveId == 4 &&
               state_.selection.fileId >= 0 &&
               state_.selection.fileId <=
                   std::numeric_limits<std::uint16_t>::max()) {
      state_.activeSprite = mediaSpriteLoader_.find(
          static_cast<std::uint16_t>(state_.selection.fileId), frameId);
    }
