...
void AssetDetailsPanel::render(
    ...
    if (state.activeAnimation.has_value()) {
        ...
        const eld::animation::AnimationData& animation =
            info.animation;
    ...
    if (state.activeSequence.has_value()) {
        ...
        ImGui::Text(
            "ID: %u",
            static_cast<unsigned int>(sequence.data.id)
        );

        ImGui::Text(
            "Frames: %zu",
            sequence.data.frames.size()
        );
        ...
        ImGui::Text(
            "Maximum loops: %u",
            static_cast<unsigned int>(
                sequence.data.maximumLoops
            )
        );

        ImGui::Text(
            "Stretches: %s",
            sequence.data.stretches ? "yes" : "no"
        );

        for (
            std::size_t index = 0;
            index < sequence.data.frames.size();
            ++index
        ) {
