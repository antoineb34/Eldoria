...
std::optional<CacheTreeNode> makeArchiveNode(
    ...
    std::optional<
        eld::spot_animation::SpotAnimationLoader
    > spotAnimationLoader;
    ...
    for (
        ...
        if (
            messageAnimationLoader.has_value() &&
            name.has_value() &&
            *name == "mesanim.dat"
        ) {
            node.children.push_back(
