...
SpotAnimationPipeline::SpotAnimationPipeline(
...
SpotAnimation SpotAnimationPipeline::get(
    std::uint16_t id
) const {
...
std::optional<SpotAnimation> SpotAnimationPipeline::find(
    std::uint16_t id
) const {
    if (!contains(id)) {
        return std::nullopt;
...
SpotAnimationPipeline::listIds() const {
...
bool SpotAnimationPipeline::contains(
    std::uint16_t id
) const {
    return id < count();
}
...
std::size_t SpotAnimationPipeline::count() const {
    const eld::archive::ArchiveFile& dataFile =
        archive_.get(DataFile);
