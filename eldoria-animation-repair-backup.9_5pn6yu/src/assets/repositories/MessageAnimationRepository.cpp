...
MessageAnimationPipeline::MessageAnimationPipeline(
...
MessageAnimation MessageAnimationPipeline::get(
    std::uint16_t id
) const {
...
std::optional<MessageAnimation> MessageAnimationPipeline::find(
    std::uint16_t id
) const {
    if (!contains(id)) {
        return std::nullopt;
...
MessageAnimationPipeline::listIds() const {
...
bool MessageAnimationPipeline::contains(
    std::uint16_t id
) const {
    return id < count();
}
...
std::size_t MessageAnimationPipeline::count() const {
    const eld::archive::ArchiveFile& dataFile =
        archive_.get(DataFile);
