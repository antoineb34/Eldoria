...
#include "repositories/SpotAnimationPipeline.h"
...
#include "repositories/MessageAnimationPipeline.h"
...
CacheTreeNode makeMediaSpriteNode(
    ...
    const eld::sprite::SpritePipeline& repository
) {
...
CacheTreeNode makeSpotAnimationGroupNode(
    ...
    std::uint16_t archiveId,
    const eld::archive::ArchiveFile& file,
    const eld::spot_animation::SpotAnimationPipeline& repository
) {
    CacheTreeNode node;
...
CacheTreeNode makeSequenceGroupNode(
    ...
    std::uint16_t archiveId,
    const eld::archive::ArchiveFile& file,
    const eld::sequence::SequencePipeline& repository
) {
    CacheTreeNode node;
...
std::optional<CacheTreeNode> makeArchiveNode(
    ...
    std::optional<
        eld::sequence::SequencePipeline
    > sequencePipeline;

    std::optional<
        eld::spot_animation::SpotAnimationPipeline
    > spotAnimationPipeline;
    ...
    std::optional<
        eld::message_animation::MessageAnimationPipeline
    > messageAnimationPipeline;
    ...
    if (entry.fileId == 2) {
        ...
        sequencePipeline.emplace(
            cache
        );

        spotAnimationPipeline.emplace(
            cache
        );
        ...
        messageAnimationPipeline.emplace(
            cache
        );
    }
    ...
    for (
        ...
        if (
            messageAnimationPipeline.has_value() &&
            name.has_value() &&
            *name == "mesanim.dat"
        ) {
            node.children.push_back(
        ...
        if (
            spotAnimationPipeline.has_value() &&
            name.has_value()
        ) {
