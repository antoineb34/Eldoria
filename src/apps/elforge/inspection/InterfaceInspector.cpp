...
void appendInterfaceDump(std::ostringstream &stream,
                         const eld::interface::WidgetData &widget,
                         const eld::interface::WidgetLoader &repository,
                         int depth, int x, int y) {
  const std::string indent(static_cast<std::size_t>(depth) * 2, ' ');

  stream << indent << "id=" << widget.id
         << " type=" << static_cast<int>(widget.type)
         << " action=" << static_cast<int>(widget.actionType)
         << " content=" << widget.contentType << " pos=" << x << "," << y
  ...
  if (widget.modelId.has_value()) {
    stream << indent << "  model=" << *widget.modelId
           << " zoom=" << widget.modelZoom << " rot=" << widget.modelRotationX
  ...
  if (!widget.itemIds.empty()) {
    ...
    stream << indent << "  items=" << widget.itemIds.size()
           << " nonEmpty=" << nonEmpty
  ...
  if (!widget.inventorySprites.empty()) {
    stream << indent << "  inventorySprites=" << widget.inventorySprites.size()
           << "\n";

    for (const eld::interface::WidgetSpriteSlot &slot :
         widget.inventorySprites) {
      stream << indent << "    slot=" << static_cast<int>(slot.slot)
             << " pos=" << slot.x << "," << slot.y << " sprite=\""
             << slot.sprite << "\"\n";
    }
  }
  ...
  for (const eld::interface::WidgetChild &child : widget.children) {
    ...
    if (!childWidget.has_value()) {
      stream << indent << "  missing-child id=" << child.id
             << " pos=" << child.x << "," << child.y << "\n";

      continue;
    }

    appendInterfaceDump(stream, *childWidget, repository, depth + 1,
                        x + child.x, y + child.y);
  }
}
...
std::string
InterfaceInspector::inspect(const eld::interface::WidgetData &root,
                            const eld::interface::WidgetLoader &repository) {
  ...
  appendInterfaceDump(stream, root, repository, 0, 0, 0);
