...
const char *transformTypeName(eld::animation::TransformType type) {
  switch (type) {
...
void writeInfo(std::ostream &output, const AnimationInspection &info,
               int indent) {
  ...
  writeIndent(output, indent + 2);
  output << "\"frameCount\": " << animation.frames.size() << ",\n";

  writeIndent(output, indent + 2);
  output << "\"skeleton\": {\n";
  writeIndent(output, indent + 4);
  output << "\"slotCount\": " << animation.skeleton.size() << ",\n";
  writeIndent(output, indent + 4);
  output << "\"slots\": [\n";

  for (std::size_t index = 0; index < animation.skeleton.size(); ++index) {
    const eld::animation::SkeletonSlot &slot = animation.skeleton[index];
    ...
    output << "]}";
    output << (index + 1 == animation.skeleton.size() ? "\n" : ",\n");
  }
  ...
  for (std::size_t index = 0; index < animation.frames.size(); ++index) {
    const eld::animation::AnimationFrameData &frame = animation.frames[index];
    ...
    for (std::size_t transformIndex = 0;
         transformIndex < frame.transforms.size(); ++transformIndex) {
      const eld::animation::FrameTransform &transform =
          frame.transforms[transformIndex];

      writeIndent(output, indent + 8);
      output << "{\"slot\": " << transform.slot << ", \"x\": " << transform.x
             << ", \"y\": " << transform.y << ", \"z\": " << transform.z << "}";
    ...
    writeIndent(output, indent + 4);
    output << "}";
    output << (index + 1 == animation.frames.size() ? "\n" : ",\n");
  }
  ...
  for (std::size_t index = 0; index < info.sequences.size(); ++index) {
    const AnimationSequenceReference &sequence = info.sequences[index];

    writeIndent(output, indent + 4);
    output << "{\"id\": " << sequence.sequenceId
           << ", \"matchingPrimaryFrames\": " << sequence.matchingPrimaryFrames
           << ", \"matchingSecondaryFrames\": "
  ...
  for (std::size_t index = 0; index < info.uses.size(); ++index) {
    ...
    writeIndent(output, indent + 4);
    output << "{\"source\": \"" << jsonEscape(use.source)
           << "\", \"id\": " << use.sourceId << ", \"name\": \""
           << jsonEscape(use.sourceName) << "\", \"role\": \""
           << jsonEscape(use.role) << "\", \"sequenceId\": " << use.sequenceId;

    if (use.viaSpotAnimationId.has_value()) {
      output << ", \"viaSpotAnimationId\": " << *use.viaSpotAnimationId;
    }

    output << ", \"provenance\": \"" << jsonEscape(use.provenance) << "\"}";
...
bool openOutput(const std::filesystem::path &path, std::ofstream &output,
                std::string &error) {
  try {
...
std::filesystem::path defaultAnimationExportPath(std::uint16_t animationId) {
  return std::filesystem::path("exports") / "animation" /
...
bool exportAnimationInspection(const AnimationInspection &info,
...
bool exportAllAnimationInspections(const AnimationInspector &relations,
                                   const std::filesystem::path &path,
                                   std::string &error) {
  std::ofstream output;
  ...
  const std::vector<std::uint16_t> ids = relations.listIds();
  ...
  try {
    for (std::size_t index = 0; index < ids.size(); ++index) {
      const AnimationInspection info = relations.inspect(ids[index]);
