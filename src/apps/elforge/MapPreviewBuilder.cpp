...
MapPreview MapPreviewBuilder::build(
    ...
    MapPreview preview;
    preview.indexEntry = *indexEntry;
    preview.centerRegion =
        loader_.load(regionId);

    preview.missingNeighborRegionIds =
    ...
    const std::vector<eld::graphics::map::SceneLocPlacement>
        sceneLocs =
            locBuilder.build(
                preview.centerRegion.objects,
                locations_,
                locSampler
            );

    preview.sceneLocs = sceneLocs;
    ...
    preview.scene.camera.verticalFov = 0.96f;
    preview.scene.camera.nearPlane = 0.5f;
    preview.scene.camera.farPlane = 512.0f;
    ...
    for (
        ...
        preview.terrainObjectIndices[plane] =
            preview.scene.objects.size();

        preview.scene.objects.push_back({
            terrainHandle,
            mapTransform,
            false
        });

        preview.locObjectIndices[plane] =
            preview.scene.objects.size();
    ...
    preview.cameraVariants.reserve(
    ...
    for (
        ...
        preview.cameraVariants.push_back({
            variant.scenePlane,
            variant.rotation,
            variant.sceneX,
            variant.sceneZ,
    ...
    preview.stats.neighborhoodRegions =
        neighborhood.regions.size();

    preview.stats.locPlacements =
        sceneLocs.size();

    preview.stats.locModelInstances =
        locModels.stats.instances;

    preview.stats.locModelParts =
        locModels.stats.parts;

    preview.stats.locModelVariants =
    ...
    for (
        ...
        preview.stats.terrainTriangles +=
            terrainBuild.stats.triangles;

        preview.stats.terrainDrawBuckets +=
            terrainBuild.stats.drawBuckets;
    }

    preview.stats.locTriangles =
        locRender.stats.triangles;

    preview.stats.locDrawBuckets =
        locRender.stats.staticDrawBuckets;

    preview.stats.cameraDependentParts =
        locRender.stats.cameraDependentParts;

    preview.stats.buildMilliseconds =
