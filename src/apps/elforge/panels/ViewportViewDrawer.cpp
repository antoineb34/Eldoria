...
ViewportViewKind ViewportViewDrawer::kindFor(
...
void ViewportViewDrawer::update(
...
ViewportDrawerLayout ViewportViewDrawer::updateLayout(
...
void ViewportViewDrawer::renderResizeHandle(
...
void ViewportViewDrawer::render(
    CacheExplorerState& state,
    ViewportViewKind kind,
    float drawerHeight,
    ...
    if (drawerHeight > CollapsedHeight + 20.0f) {
        ImGui::Separator();
        renderActivePanel(
            state,
            kind,
...
ViewportViewDrawer::interfaceOptions() const {
    return interfaceViewPanel_.options();
}

const ModelViewOptions&
ViewportViewDrawer::modelOptions() const {
    return modelViewPanel_.options();
}

const TextureViewOptions&
ViewportViewDrawer::textureOptions() const {
    return textureViewPanel_.options();
}

const char* ViewportViewDrawer::titleFor(
    ViewportViewKind kind
) {
    switch (kind) {
...
void ViewportViewDrawer::renderActivePanel(
    CacheExplorerState& state,
    ViewportViewKind kind,
    const std::function<void()>& renderAnimationControls,
    const std::function<void()>& renderMidiControls
    ...
    switch (kind) {
        case ViewportViewKind::Map:
            if (state.activeMap.has_value()) {
                const MapPreview& map = *state.activeMap;
...
const char* ViewportViewDrawer::transformTypeName(
    std::uint8_t type
) {
    switch (type) {
        case 0:
...
void ViewportViewDrawer::renderAnimationArchiveView(
    CacheExplorerState& state
) {
    if (!state.activeAnimation.has_value()) {
        return;
    ...
    const AnimationRelationsInfo& info = *state.activeAnimation;
