...
MapOverlayPoint projectMapOverlayPoint(
    ...
    return {
        ...
        point.depth >= preview.scene.camera.nearPlane &&
            point.depth <= preview.scene.camera.farPlane
    };
}
...
void drawMapSelectionOverlay(
    ...
    if (
        ...
        *state.selectedMapLocIndex < preview.sceneLocs.size()
    ) {
        const eld::graphics::map::SceneLocPlacement& loc =
            preview.sceneLocs[*state.selectedMapLocIndex];
...
MapGpuViewportRenderer::~MapGpuViewportRenderer() {
    shutdown();
}

void MapGpuViewportRenderer::shutdown() {
...
bool MapGpuViewportRenderer::ensureOpenGLContext(
    int width,
    int height
) {
    if (
...
bool MapGpuViewportRenderer::ensureOutputTexture(
    SDL_Renderer* renderer,
    int width,
    int height
) {
...
bool MapGpuViewportRenderer::makeMapContextCurrent() {
    if (
        gpuWindow_ == nullptr ||
        gpuContext_ == nullptr
    ) {
...
bool MapGpuViewportRenderer::prepare(
    SDL_Renderer* renderer,
...
void MapGpuViewportRenderer::draw(
    SDL_Renderer* renderer,
    const CacheExplorerState& state
) const {
    if (
...
const std::string& MapGpuViewportRenderer::error() const {
    return error_;
}

void MapGpuViewportRenderer::flipReadbackRows(
    int width,
    int height
) {
...
void MapGpuViewportRenderer::destroyOpenGLContext() {
    if (
        backend_ != nullptr &&
        gpuWindow_ != nullptr &&
        gpuContext_ != nullptr
...
void MapGpuViewportRenderer::destroyOutputTexture() {
    if (outputTexture_ != nullptr) {
        SDL_DestroyTexture(
            outputTexture_
        );
