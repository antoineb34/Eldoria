...
void CacheExplorer::startItemActionView(
    const eld::animation::presentation::AnimationBinding &binding) {
  if (!state_.activeItem.has_value() || !animationSource_.has_value()) {
    return;
  }
...
void CacheExplorer::showItemInventoryView() {
  ...
  std::optional<eld::model::ModelData> model =
      view.build(*state_.activeItem, modelLoader_);
...
void CacheExplorer::showItemEquippedView(ItemViewGender gender) {
  if (!state_.activeItem.has_value()) {
    return;
  }

  const ItemView view;
  std::optional<eld::model::ModelData> model = view.buildEquipped(
      *state_.activeItem, gender, identityKitLoader_, modelLoader_);
  ...
  itemViewMode_ = gender == ItemViewGender::Male ? ItemViewMode::MaleEquipped
...
void CacheExplorer::selectNextWearableItem() {
  ...
  const std::uint16_t currentId = state_.activeItem->id;

  const ItemViewMode previousMode = itemViewMode_;

  const ItemView view;

  const auto isWearable = [&view](const eld::item::ItemData &item) {
    return view.hasEquippedModel(item, ItemViewGender::Male) ||
           view.hasEquippedModel(item, ItemViewGender::Female);
  };

  const auto selectItem = [this, previousMode,
                           &view](const eld::item::ItemData &item) {
    state_.selection.type = CacheTreeNodeType::Item;

    state_.selection.definitionId = static_cast<int>(item.id);

    state_.selection.name = "obj";
    state_.selection.label = "Item " + std::to_string(item.id);

    if (!item.name.empty() && item.name != "null") {
      state_.selection.label += " - " + item.name;
    }

    const std::string ItemKeyMarker = "/definitions/obj/";

    const std::size_t marker = state_.selection.key.find(ItemKeyMarker);

    if (marker != std::string::npos) {
      const std::size_t idStart = marker + ItemKeyMarker.size();

      state_.selection.key =
          state_.selection.key.substr(0, idStart) + std::to_string(item.id);
    } else {
      state_.selection.key = "obj/" + std::to_string(item.id);
    }
    ...
    if (previousMode == ItemViewMode::MaleEquipped) {
      if (view.hasEquippedModel(*state_.activeItem, ItemViewGender::Male)) {
        showItemEquippedView(ItemViewGender::Male);
        return;
      }
    } else if (previousMode == ItemViewMode::FemaleEquipped) {
      if (view.hasEquippedModel(*state_.activeItem, ItemViewGender::Female)) {
    ...
    if (previousMode != ItemViewMode::Inventory) {
      if (view.hasEquippedModel(*state_.activeItem, ItemViewGender::Male)) {
        showItemEquippedView(ItemViewGender::Male);
      } else if (view.hasEquippedModel(*state_.activeItem,
  ...
  const auto ids = itemLoader_.listIds();

  for (const auto id : ids) {
    const eld::item::ItemData item = itemLoader_.data(id);
    if (item.id > currentId && isWearable(item)) {
      selectItem(item);
      return;
    }
  }
...
void CacheExplorer::renderItemAnimationControls() {
  ...
  const eld::item::ItemData &item = *state_.activeItem;

  const ItemView view;
  const bool hasMale = view.hasEquippedModel(item, ItemViewGender::Male);
  const bool hasFemale = view.hasEquippedModel(item, ItemViewGender::Female);
  ...
  ImGui::SameLine();
  ImGui::Text("Item %u", static_cast<unsigned int>(item.id));
  ...
  ImGui::Text("View: %s", viewLabel);

  if (itemViewMode_ == ItemViewMode::Inventory ||
  ...
  const auto sequenceAvailable = [this](std::uint16_t id) {
    const auto sequence = sequenceLoader_.find(id);

    return sequence && !sequence->data.frames.empty();
  };

  ImGui::TextUnformatted("MOVEMENT");

  if (sequenceAvailable(808) && ImGui::Button("Idle")) {
    clearNpcActionView();
    startAnimationView(std::uint16_t{808});
  ...
  const auto hasAuthoredAction = [&](Action action) {
    return std::any_of(
  ...
  const std::vector<eld::animation::presentation::AnimationBinding>
      generatedActions = ItemAnimationPresets::actions(item);

  for (const eld::animation::presentation::AnimationBinding &binding :
       generatedActions) {
    if (hasAuthoredAction(binding.action) || !binding.sequenceId.has_value() ||
        !sequenceAvailable(*binding.sequenceId)) {
      continue;
    }

    renderActionButton(actionBindingLabel(binding), binding);
  }
  ...
  if (!hasAuthoredAction(Action::Defend)) {
    ...
    if (defendSequence.has_value() && sequenceAvailable(*defendSequence)) {
  ...
  if (!hasAuthoredAction(Action::Death) && sequenceAvailable(836)) {
  ...
  for (const eld::animation::presentation::AnimationBinding &binding :
       profile.bindings) {
    if (binding.action == Action::Idle || binding.action == Action::Walk ||
  ...
  if (!hasActions) {
    ImGui::TextUnformatted("No item action animation is known for this item.");
  }

  if (activeItemAction_.has_value()) {
    const std::string actionLabel = actionBindingLabel(*activeItemAction_);
