...
GLint textureFilter(
    eld::graphics::TextureFilter filter
) {
    switch (filter) {
...
GLint textureAddressMode(
    ...
    switch (mode) {
        case eld::graphics::TextureAddressMode::Clamp:
            return GL_CLAMP_TO_EDGE;
        case eld::graphics::TextureAddressMode::Repeat:
        default:
            return GL_REPEAT;
    }
}
...
OpenGLRenderBackend::ensureModel(
    eld::graphics::ModelHandle handle,
...
GLuint OpenGLRenderBackend::ensureTexture(
    eld::graphics::TextureHandle handle,
...
void OpenGLRenderBackend::configureMaterial(
    ...
    switch (material.alphaMode) {
        case eld::graphics::AlphaMode::Blended:
            gl_.enable(GL_BLEND);
            gl_.depthMask(GL_TRUE);
            gl_.uniform1i(alphaModeLocation_, 2);
            break;

        case eld::graphics::AlphaMode::Masked:
            gl_.disable(GL_BLEND);
            gl_.depthMask(GL_TRUE);
            gl_.uniform1i(alphaModeLocation_, 1);
            break;

        case eld::graphics::AlphaMode::Opaque:
        default:
            gl_.disable(GL_BLEND);
            gl_.depthMask(GL_TRUE);
