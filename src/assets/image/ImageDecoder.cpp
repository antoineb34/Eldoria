...
void skipFrame(
    ...
    if (!data.canRead(pixelCount)) {
        throw std::runtime_error(
            "ImageData frame exceeds data payload"
        );
    }
...
ImageData ImageDecoder::decode(
    std::span<const std::uint8_t> payload
) const {
    if (payload.empty()) {
        throw std::runtime_error(
            "ImageData payload is empty"
        );
    }
    ...
    if (
        ...
        throw std::runtime_error(
            "ImageData payload is too large"
        );
    }
    ...
    if (
        ...
        throw std::runtime_error(
            "ImageData dimensions are unsupported"
        );
    }
...
ImageData ImageDecoder::decode(
    ...
    if (
        ...
        throw std::runtime_error(
            "ImageData dimensions must be positive"
        );
    }
    ...
    if (!data.canRead(pixelCount)) {
        throw std::runtime_error(
            "ImageData pixel data exceeds payload"
        );
    }
...
std::size_t ImageDecoder::frameCount(
    std::span<const std::uint8_t> dataPayload,
    std::span<const std::uint8_t> indexPayload
) const {
    eld::binary::ByteReader data(dataPayload);
    ...
    while (!data.atEnd()) {
        if (count > std::numeric_limits<std::uint16_t>::max()) {
            throw std::runtime_error("Too many image frames");
        }
        skipFrame(data, index);
        ++count;
    }
