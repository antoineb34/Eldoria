...
bool dumpActiveAsset(
    ...
    try {
        ...
        if (state.activeSequence.has_value()) {
            ...
            out << "ID: " << value.data.id << "\n";
            out << "Frames: "
                << value.data.frames.size() << "\n";
            if (value.data.frameStep.has_value()) {
                out << "Frame step: "
                    << *value.data.frameStep << "\n";
            }
            out << "Priority: "
                << static_cast<unsigned int>(value.data.priority)
                << "\n";
            out << "Maximum loops: "
                << static_cast<unsigned int>(value.data.maximumLoops)
                << "\n";
            out << "Stretches: "
                << (value.data.stretches ? "yes" : "no")
                << "\n";

            for (
                std::size_t index = 0;
                index < value.data.frames.size();
                ++index
            ) {
                const auto& frame =
                    value.data.frames[index];
        ...
        if (state.activeNpc.has_value()) {
            ...
            if (value.data.idleAnimationId.has_value()) {
                out << "Idle animation: "
